...
As of PHP 4.3.0 PCNTL uses ticks as the signal handle callback mechanism, which
...
the signal handler to function properly.
*/
declare(ticks = 1);

/*
Create a socket, bind it to localhost:9009, and start istening. Runs once in
the parent; all forked children inherit the socket's file descriptor.
*/
$acceptor = socket_create(AF_INET, SOCK_STREAM, 6);
socket_set_option($acceptor, SOL_SOCKET, SO_REUSEADDR, 1);
socket_bind($acceptor, '127.0.0.1', 9004);
...
for($i = 0; $i < 1; $i++):
  $pid = pcntl_fork();
  /*
  pcntl_fork() returns 0 in the child process and the child's process id in the
  parent. So if pid == 0 then we're in the child process.
  */
  if($pid == 0):
    // ... inside child process
    $childpid = posix_getpid();
    //printf("Child %d listening on 127.0.0.1:9009".PHP_EOL, $childpid);
    while(1):
      ...
      socket_close($socket);
      //echo "memory: ". memory_get_usage()/1024.0 . "k\n";
...
/*
Trap (Ctrl-C) interrupts, write a note, and exit immediately in parent. This
trap is not inherited by the forks because it runs after forking has commenced.
*/
